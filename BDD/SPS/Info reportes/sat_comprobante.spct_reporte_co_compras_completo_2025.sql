...
IF OBJECT_ID('[dbo].[spct_reporte_co_compras_completo_2025]') IS NOT NULL
BEGIN
    -- Generar nombre de backup con formato: NombreSP_BK_DD_Mon_YYYY
    DECLARE @NombreBK NVARCHAR(255) = 'spct_reporte_co_compras_completo_2025_BK_' + REPLACE(CONVERT(VARCHAR, GETDATE(), 106), ' ', '_');
    ...
    BEGIN
        EXEC sp_rename 'spct_reporte_co_compras_completo_2025', @NombreBK;
        PRINT '>>> BACKUP GENERADO: ' + @NombreBK;
    END
    ELSE
    BEGIN
        PRINT '>>> BACKUP EXISTENTE: ' + @NombreBK + ' (Se omite nuevo respaldo)';
        DROP PROCEDURE [dbo].[spct_reporte_co_compras_completo_2025];
    END
END
GO

CREATE PROCEDURE [dbo].[spct_reporte_co_compras_completo_2025]       
------------------------------         
--Jaime Lucas 15/12/2025 ------------------  
...
BEGIN        
    ...
    SET @params = CONCAT(
        'EXEC [dbo].[spct_reporte_co_compras_completo_2025] ',
    ...
    BEGIN TRY
        ...
        SELECT        
            ...
            IBUA,      
            RecargoGlobal,         
            TotalFactura AS TotalDocumento,        
            EstadomySatcom,        
            CUFE,       
            EstadoDIAN,      
            FechaAutorizacion,      
