...
export default function SeqMonitor() {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter(log => {
      ...
      const message = (log.RenderedMessage || log.MessageTemplate || '').toLowerCase();
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] text-neutral-200 border border-neutral-800 rounded-xl overflow-hidden shadow-2xl relative">
      ...
      <div className="absolute top-4 right-4 z-50 flex flex-col gap-2 max-w-sm pointer-events-none">
        {toasts.map(toast => (
          <div
            key={toast.id}
            className={`p-3 rounded-lg border shadow-lg flex items-start gap-2 animate-slide-in pointer-events-auto bg-[#131313] ${
      ...
      {/* Header del Monitor */}
      <header className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between border-b border-neutral-800 bg-[#111] p-4 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white tracking-wider uppercase">Seq Monitor</h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className={`w-1.5 h-1.5 rounded-full ${
                connectionStatus === 'connected' ? 'bg-emerald-500 animate-pulse' :
                connectionStatus === 'connecting' ? 'bg-amber-500 animate-pulse' :
                connectionStatus === 'error' ? 'bg-red-500' : 'bg-neutral-600'
              }`} />
              <span className="text-[10px] text-neutral-400 font-medium">{connectionStatusText}</span>
            </div>
          </div>
        </div>

        {/* Navegación Tabs */}
        <div className="flex bg-[#181818] border border-neutral-800 p-0.5 rounded-lg shrink-0 self-start sm:self-center">
          <button
            onClick={() => setActiveTab('monitor')}
            className={`px-3 py-1.5 text-xs font-bold transition-all rounded-md ${
              activeTab === 'monitor' ? 'bg-[#71BF44] text-[#131313]' : 'text-neutral-400 hover:text-white'
            }`}
          >
            Monitor
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold transition-all rounded-md ${
              activeTab === 'tasks' ? 'bg-[#71BF44] text-[#131313]' : 'text-neutral-400 hover:text-white'
            }`}
          >
            Tareas
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold transition-all rounded-md ${
              activeTab === 'connections' ? 'bg-[#71BF44] text-[#131313]' : 'text-neutral-400 hover:text-white'
            }`}
          >
            Conexiones
          </button>
      ...
      <div className="flex-1 flex overflow-hidden min-h-[550px]">
        {activeTab === 'monitor' && (
          <>
            ...
            <aside className="w-64 border-r border-neutral-800 bg-[#111] hidden md:flex flex-col p-4 shrink-0 overflow-y-auto gap-5">
              
              {/* Conexión activa */}
              <section className="flex flex-col gap-2.5">
                <h4 className="text-[10px] font-bold text-[#71BF44] tracking-wider uppercase">Conexión Activa</h4>
                <form onSubmit={handleConnect} className="flex flex-col gap-2">
                  <select
                    ...
                    className="w-full bg-[#181818] border border-neutral-800 rounded-lg p-2 text-xs text-white focus:outline-none focus:border-[#71BF44]"
                  >
                  ...
                  <button
                    ...
                    className="w-full bg-[#71BF44] text-[#131313] hover:bg-[#71BF44]/90 disabled:opacity-50 disabled:cursor-not-allowed text-xs font-bold py-2 px-3 rounded-lg flex items-center justify-center gap-1.5 transition-colors"
                  >
                    <Play className="w-3.5 h-3.5 fill-[#131313]" />
                    Conectar
                  </button>
                </form>
              </section>
              ...
              <section className="flex-1 flex flex-col min-h-0 gap-2.5">
                <div className="flex items-center justify-between">
                  <h4 className="text-[10px] font-bold text-[#71BF44] tracking-wider uppercase">Queries Guardadas</h4>
                  <button
                    onClick={() => {
                      setQueryNameInput('');
                    ...
                    return (
                      <div
                        ...
                        className={`group flex items-start justify-between p-2 rounded-lg cursor-pointer transition-all border ${
                          isActive 
                            ? 'bg-[#71BF44]/10 border-[#71BF44]/30 text-white font-medium' 
                            : 'bg-[#181818]/50 border-transparent text-neutral-400 hover:text-neutral-200 hover:border-neutral-800'
                        }`}
                        title={q.filter || 'Sin filtro'}
                      >
                        <div className="flex flex-col min-w-0 pr-1">
                          <span className="text-xs truncate">{q.name}</span>
                          <span className="text-[9px] text-neutral-500 truncate">{q.filter || 'Todos los logs'}</span>
                        </div>
                        {!q.id.startsWith('q-') && (
                          <button
                            onClick={(e) => {
                            ...
                            className="opacity-0 group-hover:opacity-100 text-neutral-500 hover:text-red-400 transition-all p-0.5 shrink-0"
            ...
            {/* Panel Principal */}
            <main className="flex-1 flex flex-col overflow-hidden bg-[#0d0d0d] p-4 gap-4">
              
              {/* Barra de Búsqueda y Parámetros */}
              <div className="bg-[#111] border border-neutral-800 rounded-xl p-3 flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row gap-2">
                  <div className="flex-1 relative">
                    ...
                    <input
                      ...
                      className="w-full bg-[#181818] border border-neutral-800 rounded-lg pl-9 pr-3 py-2 text-xs text-white focus:outline-none focus:border-[#71BF44]"
                    />
                  </div>
                  
                  <div className="flex items-center gap-2">
                    <label className="text-[10px] text-neutral-400 font-bold uppercase tracking-wider">Límite</label>
                    <input
                      type="number"
                      value={limit}
                      onChange={(e) => {
                      ...
                      className="w-16 bg-[#181818] border border-neutral-800 rounded-lg px-2 py-2 text-xs text-white text-center focus:outline-none focus:border-[#71BF44]"
                    />
                    <button
                      onClick={() => fetchLogs(false)}
                      className="bg-[#71BF44] hover:bg-[#71BF44]/90 text-[#131313] text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1.5 transition-colors shrink-0"
                    >
                ...
                <div className="flex flex-wrap items-center justify-between border-t border-neutral-900 pt-2 gap-3 text-xs">
                  <div className="flex items-center gap-2">
                    <span className={`w-2 h-2 rounded-full ${isStreaming ? 'bg-emerald-500 animate-pulse' : 'bg-neutral-600'}`} />
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5 bg-[#181818] border border-neutral-800 px-2 py-1 rounded-lg">
                      <input
                        type="checkbox"
                        id="auto-refresh"
                        checked={isStreaming}
                        ...
                        className="rounded border-neutral-700 bg-neutral-800 text-[#71BF44] focus:ring-0"
                      />
                      <label htmlFor="auto-refresh" className="text-[10px] text-neutral-400 font-bold uppercase select-none cursor-pointer">Auto-refrescar</label>
                    </div>
                    ...
                    <select
                      ...
                      className="bg-[#181818] border border-neutral-800 disabled:opacity-50 text-[11px] rounded-lg px-2 py-1 focus:outline-none"
                    >
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded-lg border transition-colors ${
                        isStreaming 
                          ...
                          : 'border-neutral-800 bg-[#181818] text-neutral-400 hover:text-white'
                      }`}
                    ...
                    <button
                      ...
                      className="border border-neutral-800 bg-[#181818] hover:bg-neutral-800 text-neutral-400 hover:text-white p-1.5 rounded-lg transition-colors"
              ...
              <div className="bg-[#111] border border-neutral-800 rounded-xl p-3 flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] text-neutral-500 font-bold uppercase tracking-wider shrink-0">Nivel de Log:</span>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            backgroundColor: isActive ? `${LEVEL_COLORS[lvl]}15` : '#181818',
                ...
                <div className="relative max-w-xs w-full">
                  ...
                  <input
                    ...
                    className="w-full bg-[#181818] border border-neutral-800 rounded-lg pl-8 pr-3 py-1.5 text-xs text-white focus:outline-none focus:border-[#71BF44]"
                  />
                </div>
              </div>

              {/* Gráfico Analítico de Eventos */}
              <div className="bg-[#111] border border-neutral-800 rounded-xl p-3 flex flex-col">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs font-bold text-white">Frecuencia de Eventos por Nivel</h4>
              ...
              <div className="flex-1 flex flex-col border border-neutral-800 bg-[#111] rounded-xl overflow-hidden min-h-0">
                <div className="bg-[#181818] border-b border-neutral-850 px-4 py-2 flex items-center justify-between text-xs font-bold text-neutral-400">
                        ...
                        >
                          ...
                          {isExpanded && (
                            <div className="border-t border-neutral-900 bg-[#131313]/50 p-3 flex flex-col gap-3 animate-fade-in text-[11px]">
                              ...
                              <div>
                                <h5 className="text-[10px] text-[#71BF44] font-bold uppercase tracking-wider mb-1">Propiedades Estructuradas</h5>
                                <div className="bg-[#181818] border border-neutral-850 p-3 rounded font-mono overflow-x-auto max-h-64">
        ...
        {/* Pestaña de Tareas */}
        {activeTab === 'tasks' && (
          <div className="flex-1 overflow-y-auto p-6 bg-[#0d0d0d] flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-white">Tareas de Monitoreo Activo</h3>
              ...
              <button
                ...
                className="bg-[#71BF44] hover:bg-[#71BF44]/90 text-[#131313] text-xs font-bold px-4 py-2 rounded-lg flex items-center gap-1.5 transition-colors"
              >
                <Plus className="w-4 h-4" />
                Nueva Tarea
              </button>
            ...
            {tasks.length === 0 ? (
              <div className="border border-dashed border-neutral-800 rounded-xl p-12 flex flex-col items-center justify-center text-center text-neutral-500">
                <Bell className="w-12 h-12 opacity-30 mb-2" />
                <p className="text-sm font-medium">No hay tareas programadas actualmente</p>
                <p className="text-xs mt-1 max-w-sm">Crea una regla de control para buscar patrones de error recurrentes de forma automática.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {tasks.map(task => (
                  <div key={task.id} className="bg-[#111] border border-neutral-800 rounded-xl p-4 flex flex-col gap-3 justify-between">
                    <div>
                      <div className="flex items-start justify-between">
                        <div>
                          <h4 className="text-sm font-bold text-white">{task.name}</h4>
                          <span className="text-[10px] text-neutral-500 break-all">{task.seqUrl}</span>
                        </div>
                      ...
                      <div className="mt-3 flex flex-col gap-1.5 text-xs text-neutral-400">
                        {task.query && (
                          <div className="flex items-center gap-1.5">
                            <Search className="w-3.5 h-3.5 text-neutral-500 shrink-0" />
                            <span className="font-mono bg-[#181818] border border-neutral-850 px-1.5 py-0.5 rounded truncate" title={task.query}>
                              {task.query}
                            </span>
                          </div>
                        )}
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>Condición: 
                            <strong className="text-neutral-200 ml-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="truncate">Acción: 
                            <strong className="text-neutral-200 ml-1 uppercase">
                    ...
                    <div className="flex items-center justify-end gap-2 border-t border-neutral-900 pt-3">
                      <button
                        ...
                        className="border border-neutral-800 bg-[#181818] hover:bg-neutral-800 text-neutral-300 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1"
                      >
                      ...
                      <button
                        ...
                        className="border border-red-900/30 bg-red-950/10 hover:bg-red-950/30 text-red-400 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors flex items-center gap-1"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                        Eliminar
                      </button>
        ...
        {activeTab === 'connections' && (
          <div className="flex-1 overflow-y-auto p-6 bg-[#0d0d0d] flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-base font-bold text-white">Conexiones a Seq</h3>
                <p className="text-xs text-neutral-400 mt-1">
            ...
            {connections.length === 0 ? (
              <div className="border border-dashed border-neutral-800 rounded-xl p-12 flex flex-col items-center justify-center text-center text-neutral-500">
                ...
                <p className="text-sm font-medium">No hay servidores configurados</p>
                <p className="text-xs mt-1">Agrega tu primer servidor de Seq para comenzar a monitorear eventos.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {connections.map(conn => (
                  <div key={conn.id} className="bg-[#111] border border-neutral-800 rounded-xl p-4 flex flex-col gap-3 justify-between">
                    <div>
                      <div className="flex items-start justify-between">
                        <h4 className="text-sm font-bold text-white">{conn.name}</h4>
                        <Server className="w-4 h-4 text-[#71BF44] opacity-80" />
                      </div>
                      <p className="text-xs text-neutral-400 mt-2 font-mono break-all">{conn.url}</p>
                      <span className="inline-block mt-3 text-[10px] text-neutral-500 font-bold bg-neutral-900 border border-neutral-850 px-2 py-0.5 rounded">
      ...
      {isSaveQueryModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111] border border-neutral-800 rounded-xl p-5 w-full max-w-md shadow-2xl animate-scale-in">
            <div className="flex items-center justify-between border-b border-neutral-900 pb-3 mb-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Guardar Consulta</h3>
              <button onClick={() => setIsSaveQueryModalOpen(false)} className="text-neutral-400 hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveQuery} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">Nombre de la Consulta</label>
                <input
                  type="text"
                  required
                  ...
                  className="bg-[#181818] border border-neutral-850 rounded-lg p-2.5 text-xs text-white focus:outline-none focus:border-[#71BF44]"
                />
              </div>

              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">Expresión de Filtro</label>
                <textarea
                  readOnly
                  value={currentFilter || '(Consulta vacía - todos los logs)'}
                  className="bg-neutral-900 border border-neutral-850 rounded-lg p-2.5 text-xs text-neutral-400 font-mono h-20 resize-none focus:outline-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 mt-2">
                <button
                  ...
                  className="border border-neutral-800 bg-[#181818] hover:bg-neutral-800 text-xs font-bold px-4 py-2.5 rounded-lg text-neutral-300"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="bg-[#71BF44] hover:bg-[#71BF44]/90 text-[#131313] text-xs font-bold px-4 py-2.5 rounded-lg flex items-center gap-1.5"
                >
      ...
      {isTaskModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111] border border-neutral-800 rounded-xl p-5 w-full max-w-lg shadow-2xl animate-scale-in">
            <div className="flex items-center justify-between border-b border-neutral-900 pb-3 mb-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">
                {editingTask ? 'Editar Tarea' : 'Nueva Tarea de Monitoreo'}
              </h3>
              <button onClick={() => setIsTaskModalOpen(false)} className="text-neutral-400 hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={saveTask} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">Nombre de la Tarea</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">Servidor Seq</label>
                <select
                  required
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">Consulta de Filtro</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] text-neutral-400 font-bold uppercase">Intervalo (segundos)</label>
                  <input
                    type="number"
                    min="5"
                    required
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] text-neutral-400 font-bold uppercase">Condición</label>
                  <select
                ...
                {(taskForm.condition === 'count_greater_than' || taskForm.condition === 'count_equal_to') && (
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] text-neutral-400 font-bold uppercase">Valor Límite</label>
                    <input
                      type="number"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="flex flex-col gap-1.5 sm:col-span-1">
                  <label className="text-[10px] text-neutral-400 font-bold uppercase">Acción</label>
                  <select
                ...
                {taskForm.actionType === 'webhook' && (
                  <div className="flex flex-col gap-1.5 sm:col-span-2">
                    <label className="text-[10px] text-neutral-400 font-bold uppercase">URL del Webhook</label>
                    <input
                      type="url"
                      required
              ...
              <div className="flex items-center justify-end gap-2 mt-4 border-t border-neutral-900 pt-4">
                <button
                  type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#111] border border-neutral-800 rounded-xl p-5 w-full max-w-md shadow-2xl animate-scale-in">
            <div className="flex items-center justify-between border-b border-neutral-900 pb-3 mb-4">
              ...
              <button onClick={() => setIsConnectionModalOpen(false)} className="text-neutral-400 hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={saveConnection} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">Nombre de la Conexión</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-neutral-400 font-bold uppercase">URL de Seq</label>
                <input
                  type="url"
                  required
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] text-[#71BF44] font-bold uppercase tracking-wider">API Key (Opcional)</label>
                <input
                  type="password"
