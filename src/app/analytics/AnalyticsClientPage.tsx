...
export default function AnalyticsClientPage({ isAdmin }: AnalyticsClientPageProps) {
  ...
  const monitoreoSections = [
    ...
    },
    {
      ...
      connection: {
        type: 'Redis',
        source: 'sara:metrics:live & sara:metrics:history',
        method: 'Monitoreo en tiempo real via Agente Python'
      }
    }
  ];
  ...
  const bddSections = [
    {
      id: 'consultas-recurrentes',
      name: 'Consultas Recurrentes',
      description: 'Análisis del rendimiento y consumo de tiempo de los Stored Procedures y sus parámetros.',
      href: '/analytics/consultas-recurrentes',
      icon: (
        <svg className="w-6 h-6 text-[#71BF44]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ...
      connection: {
        ...
        flowName: 'Monitoreo Procesos Consulta SP',
        flowId: 'VOfL2rAriW1s0TeQ',
        method: 'SP: consulta_tablero_monitoreo_sps_2026'
      }
    }
  ];

  const currentSections = activeTab === 'monitoreo' ? monitoreoSections : bddSections;
  const seqMonitorUrl = process.env.NEXT_PUBLIC_SEQ_MONITOR_URL;
